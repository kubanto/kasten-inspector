...
func CollectAll(c *cluster.Client, opts CollectOptions) (*Data, error) {
	...
	// Also catches raw SHA256 hex digests (64-char hex) used as image tags on some registries.
	if len(d.K10Reports) > 0 && d.K10Reports[0].K10Version != "" {
		if d.Version == "" || d.Version == "unknown" || d.Version == "None" || isHexDigest(d.Version) {
