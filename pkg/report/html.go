...
<div class="sec" id="jobs">
  ...
  <div style="display:flex;gap:10px;align-items:center;margin-bottom:12px;flex-wrap:wrap">
    ...
    <select id="job-status-filter" onchange="filterJobs(currentJobDays)" style="margin-left:8px;padding:3px 8px;font-size:11px;background:var(--s2);border:1px solid var(--b);border-radius:4px;color:var(--t)">
...
function filterJobs(days) {
  ...
  document.querySelectorAll('tr.job-row').forEach(function(row){
    ...
    if(start) {
      ...
      try { t = new Date(start.replace(' ', 'T')).getTime(); } catch(e) { t = 0; }
    }
    var inTime = cutoff === 0 || (t > 0 && t >= cutoff);
