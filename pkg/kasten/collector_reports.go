...
func collectK10Reports(c *cluster.Client, ns string) ([]K10Report, error) {
	...
	for _, item := range list.Items {
		...
		r.Stats.License = K10ReportLicense{
			Type:      GetString(lic, "type"),
			Status:    GetString(lic, "status"),
			NodeCount: GetInt(lic, "nodeCount"),
			NodeLimit: GetInt(lic, "nodeLimit"),
		}
		if exp := lic["expiry"]; exp != nil && exp != "<nil>" {
			expStr := fmt.Sprintf("%v", exp)
			if expStr != "<nil>" && expStr != "null" && expStr != "" {
				r.Stats.License.ExpiresAt = expStr
			}
		}
		// If no expiry and type is starter/enterprise, note it
