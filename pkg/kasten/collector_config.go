...
func collectPrometheus(c *cluster.Client, ns string) (PrometheusInfo, error) {
	...
	// Check for PrometheusRule CRs (alert rules for Kasten)
	prGVR := schema.GroupVersionResource{Group: "monitoring.coreos.com", Version: "v1", Resource: "prometheusrules"}
	prList, err := c.Dynamic.Resource(prGVR).Namespace(ns).List(ctx, metav1.ListOptions{})
	if err == nil && len(prList.Items) > 0 {
		info.AlertRules = true
	}
	if !info.AlertRules {
		prList, err = c.Dynamic.Resource(prGVR).Namespace("").List(ctx, metav1.ListOptions{})
		if err == nil && len(prList.Items) > 0 {
			info.AlertRules = true
		}
	}
